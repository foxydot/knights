...
class Admin extends CI_Controller {
	...
	function quote_ribbon($story_id,$quote_id = FALSE){
		$is_edit = $quote_id?TRUE:FALSE;
		$data = array(
				'page_title' => 'Quote Ribbon',
				'body_class' => 'projectList',
				'story' => $this->Admin->get_story($story_id),
				'sections' => $this->Admin->get_sections($story_id),
				'dashboard' => 'admin/quote',
				'is_edit' => $is_edit,
				'action' => '/admin/quote_ribbon/'.$story_id.'/'.$quote_id,
			);
		if($is_edit){
			$data['quote'] = $this->Admin->get_quote($quote_id);
		}
			
		if($this->input->post()){
			$this->Admin->create_history($this->input->post('story_id'));
			// story info, get id
			$section_info = explode('_',$this->input->post('quote_placement'));
			$db_data = array(
				'story_id' => $this->input->post('story_id'),
				'after_section' => $section_info[0],
				'after_subsection' => $section_info[1],
				'content' => $this->input->post('content'),
			);
			$db_data['quote_id'] = $is_edit?$this->input->post('quote_id'):FALSE;
			$quote_id = $this->Admin->add_quote($db_data);
			$this->Admin->set_updated_time_on_story($story_id);
			$this->load->helper('url');
		...
		$this->load->view('admin.tpl.php',$data);
		//capability for adding a quote ribbon
	}
